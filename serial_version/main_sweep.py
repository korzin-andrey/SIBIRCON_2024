...
def main(number_seed, out_path, susceptible, alpha, lmbd, init_infected, days,
    ...
    infected.append(len(I0))
    incidence_infected.append(len(I0))
    incubation.append(len(I0))
    incidence_incubation.append(len(I0))
    ...
    return infected, incidence_infected
...
if __name__ == '__main__':
    ...
    alpha_arr = np.linspace(0.35, 0.45, 21)
    lmbd_arr = np.linspace(0.25, 0.35, 21)
    ...
    for alpha_val in alpha_arr:
        ...
        people.loc[
            np.random.choice(people.index, round(
                len(people) * alpha_val), replace=False),
            'susceptible'] = 1
        ...
        def simulation(number_of_launches):
            with mp.Pool(number_of_launches) as pool:
                output = pool.map(partial(main, out_path=out_path, susceptible=susceptible, alpha=alpha_val,
                                        lmbd=lmbd_val, init_infected=init_infected, days=days,
        ...
        for lmbd_val in lmbd_arr:
            total_results = [[] for i in range(number_of_launches)]
            simulation(number_of_launches)
            output_runs = pd.DataFrame()
            for i in range(number_of_launches):
                total_results[i] = pd.read_csv(
                        out_path + r'incidence_{}.csv'.format(i))['data'].to_list()
                output_runs[str(i) + '_seed'] = total_results[i]
            
            sim_data = np.mean(total_results, axis=0)
            output_data[r'alpha={},lambda={}'.format(alpha_val,lmbd_val)] = sim_data
            output_data.to_csv(out_path + 'sweep_results.txt')
            output_runs.to_csv(out_path + r'alpha={},lambda={}'.format(alpha_val,lmbd_val))
